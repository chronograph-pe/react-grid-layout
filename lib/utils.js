...
export function compact(
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      l = compactItem(compareWith, l, compactType, cols, sorted, maxRows);
      if (!l) { return null; }
...
export function compactItem(
  ...
  if (l.y + l.h > maxRows) { return null; }
  return l;
}
...
export function synchronizeLayoutWithChildren(
  ...
  // Correct the layout.
  const correctedLayout = compact(correctBounds(layout, {cols: cols}), compactType, cols, maxRows);
